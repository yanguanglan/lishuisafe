	...
	<div class="cont jdtj">
		<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#e3e3e3">
          ...
          <tr>
            ...
            <td height="39" align="center"><span class="power"><a class="msg" href="#"></a>@if($value->noReadNum)<i>{{$value->noReadNum}}</i>@endif</span>@if($value->isLook == 1)<img src="{{URL::asset('images/jg.png')}}" class="jg"/>@endif</td>
          </tr>
          @endforeach
        </table>
        <p>&nbsp;</p><p>&nbsp;</p>
        ...
        <thead class="towns">
        	<tr>
                ...
                ?>
                <span class="power">@if($value->pneedjg ==1)局长@else质管科@endif：{{$value->userName}}  电话：{{ $value->pmobile }}<a class="msg" href="#"></a>@if($value->noReadNum)<i>{{$value->noReadNum}}</i>@endif</span>@if($value->isLook == 1)<img src="{{URL::asset('images/jg.png')}}" class="jg"/>@endif
                @endforeach
              </th>
            </tr>
        </thead>
