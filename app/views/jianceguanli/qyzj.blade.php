    ...
    <div class="cont jdtj">
        <table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#e3e3e3">
          <tr class="top">
            ...
            <td height="65" align="center" bgcolor="#f7f7f7"><a href="{{ URL::route('qyzjinfo', array('city'=>0, 'type'=>$typeItem->ID, 'startDate'=>$startDate, 'endTime'=>$endTime, 'keyword'=>$keyword)) }}" target="_blank">{{$typeItem->pname}}</a></td>
           @endforeach
            <td height="65" align="center" bgcolor="#f7f7f7">合计</td>
          </tr>
          @foreach ($result as $item)
          <tr>
            <td width="91" height="39" align="center" class="td01"><a href="{{ URL::route('qyzjinfo', array('city'=>$item->cityID, 'type'=>0, 'startDate'=>$startDate, 'endTime'=>$endTime, 'keyword'=>$keyword)) }}" target="_blank">{{ $item->cityName }}</a></td>
